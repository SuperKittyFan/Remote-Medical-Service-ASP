...
namespace 医院管理系统
{
	...
	public partial class index : System.Web.UI.Page
	{
		...
		protected void Button1_Click(object sender, System.EventArgs e)
		{
            if (Tname.Text == "" )
            {
                Response.Write("<script language=javascript>alert('用户名不能为空！');window.location = 'index.aspx';</script>");
            }
			else if(cpa.CH(Tname.Text,Tpwd.Text,"")=="Nopasscheck")
			{
                Response.Write("<script language=javascript>alert('你的用户名或密码有误！');window.location = 'index.aspx';</script>");
